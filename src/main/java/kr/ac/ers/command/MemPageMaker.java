...
public class MemPageMaker {
	//화면 출력 시 필요(시스템에서 정의) - service
		...
		MemberSearchCriteria memCri = new MemberSearchCriteria();

		//startPage, endPage, prev, next 설정. by totalCount
		private void calcData() {
			endPage = (int) (Math.ceil(memCri.getPage() / (double) displayPageNum) * displayPageNum);
			startPage = (endPage - displayPageNum) + 1;
			
			realEndPage = (int) (Math.ceil(totalCount / (double) memCri.getPerPageNum()));
		...
		public MemberSearchCriteria getMemCri() {
			return memCri;
		}

		public void setMemCri(MemberSearchCriteria cri) {
			this.memCri = cri;
		}
